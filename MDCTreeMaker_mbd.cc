...
/*  
  ...
  */
  ...
  {
    BbcPmtContainerV1 *pmts = findNode::getClass<BbcPmtContainerV1>(topNode, "BbcPmtContainer");
    if(pmts)
      {
	npmt = pmts->get_npmt();
	for(int i=0; i<npmt; ++i)
	  {
	    int adc = pmts->get_adc(i);
	    qsum += adc;
	  }
      }
    else cout << "no pmts!" << endl;
  }

  {
    PHG4HitContainer *mbdhits = findNode::getClass<PHG4HitContainer>(topNode, "G4HIT_BBC");
    if(mbdhits)
      {
	PHG4HitContainer::ConstRange hitrange = mbdhits->getHits();
	for(PHG4HitContainer::Map::const_iterator it = hitrange.first; it != hitrange.second; ++it)
	  {
	    float edep = it->second->get_edep();
	    mbdsumE += edep;
	  }
      }
    else cout << "No hitcontainer!" << endl;
  }
